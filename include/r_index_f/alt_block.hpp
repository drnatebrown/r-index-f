...
public:
    ...
    interval_pos LF_prior(interval_pos pos, uchar c)
    {
        ...
        ulint prior_run = first_block_run(curr_run) + get_block(curr_run).run_heads.select(c_rank + 1, c);
    ...
    interval_pos LF_next(interval_pos pos, uchar c)
    {
        ...
        while((curr_run / block_size) < blocks.size() - 1 && c_rank + 1 > get_block(curr_run).run_heads.rank(block_size, c)) {
            curr_run = ((curr_run/block_size) + 1) * block_size;
            // b = &get_block(curr_run);
            c_rank = 0;
        }
        if (c_rank + 1 > get_block(curr_run).run_heads.rank(block_len(curr_run), c)) {
            return interval_pos();
        }
...
private:
    ...
    interval_pos LF(ulint k, ulint d, uchar c, ulint c_rank) {
        // block* b = &get_block(k);
        ulint next_interval = get_block(k).dest_pred.get(c, c_rank);
        ulint next_off = get_block(k).dest_off[row(k)] + d;
